...
class my_robot(object):
    def __init__(self):
        ...
        self.suction_enable = False  # initial suction
    ...
    def get_joint_pos(self):
        ...
        for i in range(jointNum):
            err, self.joint_pos[i] = vrep.simxGetJointPosition(self.clientID, self.joint_handle[i], ONESHOT)
    ...
    def enable_suction(self, active):
        if active:
            vrep.simxSetIntegerSignal(self.clientID, 'suctionActive', 1, BLOCKING)
            self.suction = vrep.simxGetIntegerSignal(self.clientID, 'suctionActive', ONESHOT)
        else:
            vrep.simxClearIntegerSignal(self.clientID, 'suctionActive', BLOCKING)
            self.suction = vrep.simxGetIntegerSignal(self.clientID, 'suctionActive', ONESHOT)

    # def disable_suction(self):
    #     vrep.simxClearIntegerSignal(self.clientID,self.suctionPad,0,BLOCKING)
    #     self.suction=vrep.simxGetIntegerSignal(self.clientID,'suctionActive',ONESHOT)
