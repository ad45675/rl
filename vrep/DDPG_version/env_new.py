...
class robot_env(object):
    ...
    joint2_bound = [-80 * degtorad, 80 * degtorad]#(-1.430~0)
    joint3_bound = [-70 * degtorad, 70 * degtorad]#(-1.22~1.814)
    joint4_bound = [0 * degtorad, 0 * degtorad]
    joint5_bound = [-90* degtorad, 90 * degtorad]#(-1.57~0)
    ...
    def get_state(self):
        ...
        s = np.hstack((joint_state, distance))
        print('s',s)
        # 吸嘴狀態還沒加上去
        return s
