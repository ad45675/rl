...
class SAC_agent(object):
    ...
    def choose_action(self, observation, reparameterize=False,deterministic=False):
        state = T.Tensor([observation]).to(self.actor.device)
        actions, _ = self.actor.sample_normal(state, reparameterize=False, deterministic=False)
    ...
    def learn(self):
        ...
        with T.no_grad():
            ...
            action1, log_prob= self.actor.sample_normal(state_, reparameterize=config.reparameterize_critic,deterministic=False)  #看有沒有sample
            # Target Q-values
        ...
        action2, log_prob2 = self.actor.sample_normal(state, reparameterize=config.reparameterize_actor, deterministic=False)
