...
def train():
    ...
    file_name = ['step.txt', 'reward.txt', 'avgR.txt', 'repeat.txt']
    data_set = ['step_set', 'reward_set', 'avg_reward_set', 'repeat_set']
    for i in range(len(file_name)):
        save_txt(path=folder_data, name=file_name[i], data=eval(data_set[i]))

    # plot fig
    folder_fig = './model/' + PATH + '/train/fig/'
    xlabel = ['Episode', 'Episode', 'Episode', 'Episode', 'update times', 'update times']
    ylabel = ['step', 'r', 'avgR', 'repeat', 'cost', 'cost']
    title = ['step', 'reward', 'avgR', 'repeat', 'cost_actor', 'cost_critic']
    for i in range(len(file_name)):
